...
// for (let x = 0; x < 30; ++x) {
//   assert(CryptoUnit.fromDecimal(`1e${x}`).toBuffer() < CryptoUnit.fromDecimal(`2e${x}`).toBuffer());
//   assert(CryptoUnit.fromDecimal(`1e${x}`).toBuffer().toString() < CryptoUnit.fromDecimal(`2e${x}`).toBuffer().toString());
// }
